...
![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-1-30/46353145-file_1485748885622_b1c4.png)
...
- `column-reverce`：表示弹性项目（flex item）从下往上排列。
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-4/98238970-file_1486220966327_16a43.png)
...
- `no-wrap`：仍是沿着侧轴方向换行，只不过侧轴方向需要逆置
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/89127756-file_1486225449066_d732.png)
...
- `space-around`：分布对齐（每个 item 都平分所有空隙，所以会发现，两个元素间空隙的宽度会是最左边缝隙的宽度的两倍）
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-4/74515341-file_1486222732549_78b6.png)
...
- `baseline`：让弹性项目的第一行文字的基线对齐
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/49922052-file_1486224363785_4537.png)
...
- `stretch`：**默认值**，如果项目未设置高度，那么将延伸铺满整个容器
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/92072570-file_1486226129510_e9d2.png)
...
- 那么，如果对弹性容器设置`flex-wrap：wrap-reverse`会怎么样？
  ...
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/2452061-file_1486226179320_d907.png)
...
仍然会按照`order`顺序排列好：
![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/11769597-file_1486227165614_2df.png)
...
- 默认值是 0，下面就是默认的分配情况：
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/52349159-file_1486227621289_5082.png)

- 当为所有弹性项目都设置`flex-grow:1;`之后：
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/30839986-file_1486227710080_17083.png)

- 如果给项目 2，设置`flex-grow:2;`而其他项目都是 1，那么项目 2 分配到的剩余空间就会是其他项目的两倍：
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/13034584-file_1486227814361_b1b3.png)
...
![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/46240670-file_1486289303987_14b49.png)
...
- 然而，在此时，我发现了一个神奇的现象。当弹性项目的子元素，在主轴方向的尺寸，超过了该弹性项目，就会导致这个弹性项目的`flex-shrink`失效。当然解决方案也已经列在注释中：
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/80372936-file_1486300825907_1e48.png)
...
- 默认是`auto`，表示继承父元素的`align-items`，如果没有父元素，就等同于`stretch`
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-2-5/26112348-file_1486301845816_ad8e.png)
