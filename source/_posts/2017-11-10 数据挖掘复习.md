...
![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/10999007.jpg)
...
- 对称的和倾斜的数据：对称，正倾斜（众数小于中位数），负倾斜（众数大于中位数）
  ...
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/67206708.jpg)
...
- 方差和标准差
  ...
  - 有偏估计方差
    ...
    ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/19846478.jpg)

  - 无偏估计方差

    ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/91244837.jpg)
...
- 可视化
  ...
  - 分位数图（Quantile Plot），横轴是百分比，纵轴是数值
    ...
    ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/49705672.jpg)

  - Q-Q Plot，比较两组数据是否来自同一分布

    ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/65227165.jpg)
...
- 二元属性（binary attribute）的相异度度量：
  ...
  ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/23023651.jpg)
...
- 数值属性（numeric)：
  ...
  - 标准化：
    ...
    - 平均绝对离差（mean absolute deviation）：计算每个属性的平均值，以及每个属性的标准差，再进行 z-score 标准化，更鲁棒
      ...
      ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/33167128.jpg)
  ...
  - 上确界距离（$L_{max}$，切比雪夫距离）
    ...
    ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/9919791.jpg)
...
- 混合类型
  ...
  - 计算距离：
    ...
    ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-13/64012434.jpg)
...
- 数据集成中的冗余（redundancy）问题
  ...
  - 可以通过相关分析（correlation analysis）和协方差分析（covariance analysis）进行冗余检测
    ...
    - 相关分析：$\chi^2$卡方检验
      ...
      ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-14/41219297.jpg)
    ...
    - 相关分析：皮艾森系数（Pearson's product moment coefficient）
      ...
      ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-14/12962797.jpg)

      ​

    - 协方差分析：针对数值型数据

      协方差：![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-14/80220904.jpg)

      ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-14/48564062.jpg)

      协相关系数（correlation coefficient:）：

      ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-14/63996666.jpg)
...
8. 置信度（confidence）：包含 X 的事务同时也包含 Y 的概率，P(Y|X)
   ...
   ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-15/1487277.jpg)
...
![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-15/11954617.jpg)
...
1. 构建 fp tree：
   ...
   ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-15/53320418.jpg)

2. 条件模式基（conditional pattern bases）

   ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-15/10773567.jpg)

3. 寻找条件模式树（conditional FP-tree，类似于寻找最长公共子序列）

   ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-15/19614960.jpg)

4. 简化，前缀可以被简化成一个节点

   ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-15/95693211.jpg)
...
2. 挖掘极大模式
   ...
   ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-15/71303397.jpg)
...
- 大数据库的分类
  ...
  - 可伸缩的决策树算法，RainForest：
    ...
    - AVC-group：节点上的所有 AVC-set 的集合。
      ...
      ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-16/54708839.jpg)
...
2. 朴素贝叶斯分类
   ...
   - 假设$X$的各个属性之间相互独立，不存在依赖关系，那么
     ...
     - 如果属性是连续值，一般假设属性服从高斯分布。
       ...
       $P(x_k|C_i)=g(x_k,\mu_{C_i},\sigma_{C_i})$![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-16/66390485.jpg)

   - example

     ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-16/32057659.jpg)
...
1. 混淆矩阵：对于给定 m 个类，混淆矩阵至少是一个 m\*m 的表。以下是一个 2\*2 的混淆矩阵，纵向是实际分类，横向是预测分类。
   ...
   ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-16/68799474.jpg)
...
![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-17/11929324.jpg)
...
- 参数和基本概念：
  ...
  - 密度可达的（Density-reachable）：存在对象链 p1,…,pn，后一个是前一个直接密度可达的，那么说明 pn 是 p1 密度可达的；密度可达并不是一个等价关系，只有当 p1,pn 都是核心对象时，才一定保证可逆。
    ...
    ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-17/31501679.jpg)

  - 密度相连的（Density-connected）：存在 p1，p2，q，p1 和 q 以及 p2 和 q 都是密度可达的，那么 p1 和 p2 是密度相连的。密度相连是等价关系。

    ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-17/63923841.jpg)
...
- 评估聚类趋势（assessing clustering tendency）
  ...
  - 霍普金斯统计量（Hopkins Statistic）
    ...
    ![](http://jackie-image.oss-cn-hangzhou.aliyuncs.com/17-11-17/66309480.jpg)
